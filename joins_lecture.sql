...
# you do not need to use dot notation for this ^
...
SELECT p.person_id, a.album_name FROM persons p JOIN albums a ON p.album_id = a.user_id;
...
SELECT p.first_name, a.album_name AS name FROM persons p JOIN preferences pref ON p.person_id = pref.person_id JOIN albums a ON pref.album_id = a.user_id;